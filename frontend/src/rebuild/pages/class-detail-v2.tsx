...
function MaterialFileButtons({ item, onPreview }: Readonly<{ item: MaterialItem; onPreview: (id: string, name: string, type?: string) => void }>) {
  return (
    <div className="flex shrink-0 items-center gap-1">
      {item.fileId && (
        <>
          <Button variant="ghost" className="min-h-8 px-2" onClick={() => api.downloadFile(item.fileId!, item.title)}><Download size={14} /></Button>
          <Button variant="ghost" className="min-h-8 px-2" onClick={async () => { const meta = await api.fileMetadata(item.fileId!); onPreview(meta.id, meta.originalFileName, meta.contentType) }}><Eye size={14} /></Button>
        </>
      )}
      {item.externalUrl && <a href={item.externalUrl} target="_blank" rel="noreferrer" className="rounded-lg p-1.5 text-emerald-600 hover:bg-emerald-50"><LinkIcon size={14} /></a>}
    </div>
  )
}
...
export function ClassDetailV2Page() {
  ...
  return <div className="space-y-5"><div className="sticky top-20 z-20 space-y-3"><PageHeader eyebrow={klass.data.code} title={klass.data.name} description={isAdmin ? 'Góc nhìn quản trị lớp: chỉ hiển thị dữ liệu và thao tác trong phạm vi lớp được giao.' : klass.data.description || 'Chi tiết vận hành lớp học'} actions={<>{canOperate && <Button variant="secondary" onClick={openEdit}><Pencil size={16} /> Sửa lớp</Button>}<Button variant="secondary" onClick={() => api.downloadClassStudentsCsv(classId)}><Download size={16} /> Xuất CSV học viên</Button></>} /><div className="flex gap-2 overflow-x-auto rounded-3xl border border-sky-100 bg-white/90 p-2 shadow-sm">{tabs.map((item) => <button key={item} className={`whitespace-nowrap rounded-2xl px-3 py-2 text-sm font-bold ${tab === item ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:bg-sky-50'}`} onClick={() => setTab(item)}>{item}</button>)}</div></div>
    ...
    {tab === 'Tài liệu' && <Card><div className="mb-3 flex items-center justify-between gap-3"><h2 className="font-black">Tài liệu</h2>{canOperate && <Link className="inline-flex min-h-10 items-center gap-2 rounded-2xl border border-sky-200 bg-white px-4 text-sm font-bold text-slate-700 hover:bg-sky-50" to={`${rolePrefix}/materials`}><ArrowRight size={16} />Mở thư viện</Link>}</div><div className="mt-3 divide-y divide-sky-50">{materialsPage.items.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 py-3"><div className="min-w-0"><div className="flex items-center gap-2">{item.fileId ? <FileText size={14} className="shrink-0 text-indigo-500" /> : item.externalUrl ? <LinkIcon size={14} className="shrink-0 text-emerald-500" /> : <FileText size={14} className="shrink-0 text-slate-400" />}<b className="truncate">{item.title}</b></div><p className="text-xs text-slate-500">{item.description || 'Chưa có mô tả'}</p></div><div className="flex shrink-0 items-center gap-2"><span className={`text-xs font-bold ${item.visible ? 'text-emerald-600' : 'text-slate-400'}`}>{item.visible ? 'Hiện' : 'Ẩn'}</span><MaterialFileButtons item={item} onPreview={(id, name, type) => setPreviewFile({ id, name, type })} /></div></div>)}{materialsPage.items.length === 0 && <EmptyState title="Chưa có tài liệu" description="Tài liệu của lớp sẽ xuất hiện sau khi được tạo trong thư viện tài liệu." />}</div>{materialsPage.totalPages > 1 && <div className="mt-3"><PaginationControls page={materialPage} totalPages={materialsPage.totalPages} onPageChange={setMaterialPage} /></div>}</Card>}
