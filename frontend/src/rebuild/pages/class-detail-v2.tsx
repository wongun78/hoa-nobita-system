...
import { ArrowRight, Download, Eye, ExternalLink, FileText, LinkIcon, Pencil, Plus, Save, Trash2, UserMinus } from 'lucide-react'
import { useNewAuth } from '../auth/use-auth'
import { ConfirmDialog, EmptyState, ErrorState, MetricCard, Modal, PageHeader, PaginationControls, StatusBadge } from '../components/foundation'
import { FilePreviewModal } from '../components/file-preview-modal'
import { api } from '../core/api'
import { ApiClientError } from '../core/http'
import type { ClassItem, ClassStatus, LessonItem, LessonStatus, MaterialItem, NotificationItem, StudentMemberItem } from '../core/types'
import { Button, Card, FieldLabel, Input, TextArea } from '../layout/ui'
import { asPage, fmtDate, numberValue } from './phase2-utils'

function MaterialFileButtons({ item, onPreview }: Readonly<{ item: MaterialItem; onPreview: (id: string, name: string, type?: string) => void }>) {
  return (
    <div className="flex shrink-0 items-center gap-2">
      {item.fileId && (
        <>
          <button type="button" className="inline-flex min-h-11 items-center gap-1 rounded-2xl bg-indigo-600 px-3 text-sm font-bold text-white transition hover:bg-indigo-700" onClick={() => api.downloadFile(item.fileId!, item.title)}><Download size={16} />Tải</button>
          <button type="button" className="inline-flex min-h-11 items-center gap-1 rounded-2xl border border-sky-200 px-3 text-sm font-bold text-slate-700 transition hover:bg-sky-50" onClick={async () => { const meta = await api.fileMetadata(item.fileId!); onPreview(meta.id, meta.originalFileName, meta.contentType) }}><Eye size={16} />Xem trước</button>
        </>
      )}
      {item.externalUrl && <a href={item.externalUrl} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center gap-1 rounded-2xl border border-sky-200 px-3 text-sm font-bold text-slate-700 transition hover:bg-sky-50"><ExternalLink size={16} />Liên kết</a>}
    </div>
  )
}
...
export function ClassDetailV2Page() {
  ...
  const lessonsSorted = useMemo(() => [...lessonsPage.items].sort((a, b) => (b.lessonDate ?? '').localeCompare(a.lessonDate ?? '')), [lessonsPage.items])
  ...
  const needGrading = submissionsPage.items.filter((item) => item.status !== 'GRADED').length
  ...
  return <div className="space-y-5"><div className="sticky top-20 z-20 space-y-3"><PageHeader eyebrow={klass.data.code} title={klass.data.name} description={isAdmin ? 'Góc nhìn quản trị lớp: chỉ hiển thị dữ liệu và thao tác trong phạm vi lớp được giao.' : klass.data.description || 'Chi tiết vận hành lớp học'} actions={<>{canOperate && <Button variant="secondary" onClick={openEdit}><Pencil size={16} /> Sửa lớp</Button>}</>} /><div className="flex gap-2 overflow-x-auto rounded-3xl border border-sky-100 bg-white/90 p-2 shadow-sm">{tabs.map((item) => <button key={item} className={`whitespace-nowrap rounded-2xl px-3 py-2 text-sm font-bold ${tab === item ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:bg-sky-50'}`} onClick={() => setTab(item)}>{item}</button>)}</div></div>
    ...
    {tab === 'Học viên' && <Card><div className="mb-3 flex items-center justify-between"><h2 className="font-black">Học viên</h2></div><div className="overflow-auto"><table className="w-full min-w-[820px] text-sm"><thead><tr className="border-b border-sky-100 text-left text-slate-500"><th className="px-3 py-3">Học viên</th><th className="px-3 py-3">Mã học viên</th><th className="px-3 py-3">Trạng thái</th><th className="px-3 py-3">Ngày vào</th><th className="px-3 py-3">Hành động</th></tr></thead><tbody>{studentsPage.items.map((student) => <StudentRow key={student.id} classId={classId} student={student} canRemove={canOperate} />)}</tbody></table></div>{studentsPage.items.length === 0 && <EmptyState title="Chưa có học viên" description="Khi học viên được thêm vào lớp, danh sách sẽ hiển thị tại đây." />}{studentsPage.totalPages > 1 && <div className="mt-3"><PaginationControls page={studentPage} totalPages={studentsPage.totalPages} onPageChange={setStudentPage} /></div>}</Card>}
    {tab === 'Buổi học' && <Card><div className="mb-3 flex items-center justify-between"><h2 className="font-black">Buổi học</h2>{canOperate && <Button onClick={openCreateLesson}><Plus size={14} /> Tạo buổi học</Button>}</div><div className="mt-3 divide-y divide-sky-50">{lessonsSorted.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 py-3"><div className="min-w-0"><b>{formatLessonLabel(item, klass.data?.name)}</b><p className="text-xs text-slate-500"><StatusBadge value={item.status} /> · {fmtDate(item.lessonDate) || 'Chưa có ngày'}</p></div>{canOperate && <div className="flex shrink-0 gap-1"><Button variant="ghost" onClick={() => openEditLesson(item)}><Pencil size={14} /></Button><Button variant="ghost" onClick={() => setLessonDeleteTarget(item)}><Trash2 size={14} className="text-rose-500" /></Button></div>}</div>)}{lessonsSorted.length === 0 && <EmptyState title="Chưa có buổi học" description="Lớp này chưa có lịch buổi học trong dữ liệu hiện tại." />}</div>{lessonsPage.totalPages > 1 && <div className="mt-3"><PaginationControls page={lessonPage} totalPages={lessonsPage.totalPages} onPageChange={setLessonPage} /></div>}</Card>}
    {tab === 'Tài liệu' && <Card><div className="mb-3 flex items-center justify-between gap-3"><h2 className="font-black">Tài liệu</h2>{canOperate && <Link className="inline-flex min-h-10 items-center gap-2 rounded-2xl border border-sky-200 bg-white px-4 text-sm font-bold text-slate-700 hover:bg-sky-50" to={`${rolePrefix}/materials`}><ArrowRight size={16} />Mở thư viện</Link>}</div><div className="mt-3 divide-y divide-sky-50">{materialsPage.items.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 py-3"><div className="min-w-0"><div className="flex items-center gap-2">{item.fileId ? <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-indigo-50 text-indigo-600"><FileText size={16} /></div> : item.externalUrl ? <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-emerald-50 text-emerald-600"><LinkIcon size={16} /></div> : <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-slate-50 text-slate-400"><FileText size={16} /></div>}<b className="truncate">{item.title}</b></div><p className="text-xs text-slate-500">{item.description || 'Chưa có mô tả'}</p></div><div className="flex shrink-0 items-center gap-2"><span className={`text-xs font-bold ${item.visible ? 'text-emerald-600' : 'text-slate-400'}`}>{item.visible ? 'Hiện' : 'Ẩn'}</span><MaterialFileButtons item={item} onPreview={(id, name, type) => setPreviewFile({ id, name, type })} /></div></div>)}{materialsPage.items.length === 0 && <EmptyState title="Chưa có tài liệu" description="Tài liệu của lớp sẽ xuất hiện sau khi được tạo trong thư viện tài liệu." />}</div>{materialsPage.totalPages > 1 && <div className="mt-3"><PaginationControls page={materialPage} totalPages={materialsPage.totalPages} onPageChange={setMaterialPage} /></div>}</Card>}
    ...
    <Modal open={editOpen} title="Sửa lớp học" onClose={() => setEditOpen(false)}>
