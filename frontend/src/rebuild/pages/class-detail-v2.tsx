...
import { ArrowRight, Download, Eye, FileText, LinkIcon, Pencil, Plus, Save, Trash2, UserMinus, X } from 'lucide-react'
    ...
    <div className="flex shrink-0 items-center gap-2">
      {item.fileId && (
        <>
          ...
          <button type="button" className="inline-flex min-h-11 items-center gap-1 rounded-2xl border border-sky-200 px-3 text-sm font-bold text-slate-700 transition hover:bg-sky-50" onClick={async () => { const meta = await api.fileMetadata(item.fileId!); onPreview(meta.id, meta.originalFileName, meta.contentType) }}><Eye size={16} />Xem</button>
        </>
      )}
      {item.externalUrl && <a href={item.externalUrl} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center gap-1 rounded-2xl border border-sky-200 px-3 text-sm font-bold text-slate-700 transition hover:bg-sky-50"><LinkIcon size={16} />Liên kết</a>}
    </div>
  )
}
