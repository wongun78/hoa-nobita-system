...
export function GradingV2Page() {
  ...
  const submissions = useQuery({ queryKey: ['grading-v2', classId, page, search, isTeacher], queryFn: () => isTeacher ? api.gradingSubmissionsPage({ page, size: 12, search, classId: classId || undefined }) : api.classGradingSubmissionsPage(classId, { page, size: 12, search }), enabled: isTeacher || Boolean(classId) })
  ...
  return <div className="space-y-5"><PageHeader eyebrow="채점 센터" title="Grading Center" description={isTeacher ? 'Teacher Owner xem toàn bộ bài nộp toàn hệ thống; bộ lọc lớp là tùy chọn. Cmd/Ctrl+Enter để lưu, Arrow Up/Down đổi bài.' : 'Class Admin chỉ xem/chấm trong lớp được gán; hãy chọn lớp để bắt đầu.'} />
    <Card className="flex flex-col gap-3 md:flex-row"><select className="rounded-2xl border border-sky-100 bg-white px-3 py-2.5 text-sm" value={classId} onChange={(e) => { setClassId(e.target.value); setPage(0); setSelectedId('') }}><option value="">{isTeacher ? 'Tất cả lớp' : 'Chọn lớp'}</option>{classPage.items.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><SearchInput value={search} onChange={(e) => { setPage(0); setSearch(e.target.value) }} placeholder="Tìm học viên/bài tập" /></Card>
    <div className="grid gap-5 xl:grid-cols-[390px_1fr]">
      <Card className="rounded-3xl">{!isTeacher && !classId && <EmptyState title="Chọn lớp để bắt đầu" />}{submissions.isLoading && <SkeletonCard />}{submissions.isError && <ErrorState onRetry={() => void submissions.refetch()} />}{(isTeacher || classId) && !submissions.isLoading && queue.items.length === 0 && <EmptyState title="Không có bài cần chấm" />}{queue.items.length > 0 && <><div className="max-h-[650px] space-y-2 overflow-auto pr-1">{queue.items.map((item) => <button key={item.id} className={`w-full rounded-2xl border p-3 text-left transition ${selectedId === item.id ? 'border-indigo-300 bg-indigo-50' : 'border-sky-100 bg-white hover:bg-sky-50'}`} onClick={() => setSelectedId(item.id)}><div className="flex items-start justify-between gap-2"><div><p className="font-bold text-slate-950">{item.studentName}</p><p className="text-xs text-slate-500">{item.assignmentTitle}</p></div><StatusBadge value={item.status} /></div><p className="mt-1 text-xs font-semibold text-slate-500">{item.className}</p><p className="mt-2 text-xs text-slate-400">{fmtDate(item.submittedAt)}</p></button>)}</div><div className="mt-4"><PaginationControls page={queue.page} totalPages={queue.totalPages} onPageChange={setPage} /></div></>}</Card>
