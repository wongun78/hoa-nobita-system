...
export function ClassesPage() {
  ...
  return (
    <div className="space-y-5">
      <PageHeader eyebrow="클래스 운영" title={isAdmin ? 'Lớp được giao' : 'Lớp học'} description={isAdmin ? 'CLASS_ADMIN chỉ thấy lớp được phân quyền và không có action global-only.' : 'Tìm kiếm, lọc và quản lý lớp học theo phân trang server-side.'} actions={canCreate && <Button onClick={() => setShowCreate(true)}><Plus size={16} /> Tạo lớp</Button>} />
