  ...
  return (
    <dialog open className="fixed inset-0 z-50 m-0 grid h-full w-full max-w-none place-items-center bg-slate-950/30 p-4 backdrop-blur-sm">
      <Card className="w-full max-w-2xl shadow-2xl">
        ...
        {error && <div className="mt-3 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-700">{error}</div>}
        <form className="mt-4 grid gap-3 sm:grid-cols-2" onSubmit={(e) => { e.preventDefault(); if (fullName.trim()) create.mutate() }}>
          ...
          <div><FieldLabel htmlFor="role">Vai trò</FieldLabel><select id="role" className="w-full rounded-2xl border border-sky-100 bg-white px-3 py-2.5 text-sm" value={role} onChange={(e) => setRole(e.target.value as RoleName)}><option value="STUDENT">Học viên</option><option value="CLASS_ADMIN">Trợ giảng</option><option value="TEACHER_OWNER">Giảng viên chính</option></select></div>
...
export function UsersPage() {
  ...
  const statusMutation = useMutation({
    mutationFn: ({ id, next }: { id: string; next: UserStatus }) => api.updateUserStatus(id, next),
    onMutate: async ({ id, next }) => {
      await qc.cancelQueries({ queryKey: ['users'] })
      const previousPages = qc.getQueriesData<import('../core/types').PageResponse<import('../core/types').UserItem>>({ queryKey: ['users'] })
      qc.setQueriesData<import('../core/types').PageResponse<import('../core/types').UserItem>>({ queryKey: ['users'] }, (old) => {
        if (!old || !('items' in old)) return old
        return { ...old, items: old.items.map((u) => u.id === id ? { ...u, status: next } : u) }
      })
      return { previousPages }
    },
    onError: (_err, _vars, ctx) => {
      if (ctx?.previousPages) {
        for (const [key, data] of ctx.previousPages) {
  ...
  return (
    <div className="space-y-5">
      <PageHeader eyebrow="QUẢN LÝ NGƯỜI DÙNG" title="Người dùng" description="Quản lý học viên, trợ giảng và tài khoản giáo viên." actions={<Button onClick={() => setDialogOpen(true)}><Plus size={16} /> Tạo người dùng</Button>} />
      {temporaryPassword && <Card className="border-emerald-200 bg-emerald-50"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-bold text-emerald-950">Mật khẩu tạm thời</p><code className="text-sm text-emerald-700">{temporaryPassword}</code></div><Button variant="secondary" onClick={() => navigator.clipboard.writeText(temporaryPassword)}><Copy size={16} /> Sao chép</Button></div></Card>}
      <FilterBar><SearchInput value={search} onChange={(e) => { setPage(0); setSearch(e.target.value) }} placeholder="Tìm tên, email, số điện thoại" /><select className="rounded-2xl border border-sky-100 bg-white px-3 py-2.5 text-sm" value={role} onChange={(e) => { setPage(0); setRole(e.target.value) }}><option value="">Tất cả vai trò</option><option value="TEACHER_OWNER">Giảng viên chính</option><option value="CLASS_ADMIN">Trợ giảng</option><option value="STUDENT">Học viên</option></select><select className="rounded-2xl border border-sky-100 bg-white px-3 py-2.5 text-sm" value={status} onChange={(e) => { setPage(0); setStatus(e.target.value) }}><option value="">Tất cả trạng thái</option><option value="ACTIVE">Hoạt động</option><option value="INACTIVE">Ngưng</option><option value="SUSPENDED">Đã khoá</option></select></FilterBar>
...
export function UserDetailPage() {
  ...
  return <div className="space-y-5"><PageHeader eyebrow="CHI TIẾT NGƯỜI DÙNG" title={user.data.fullName} description={user.data.email || user.data.phone || 'Không có thông tin liên hệ'} actions={<Link className="rounded-2xl border border-sky-200 bg-white px-4 py-2 text-sm font-bold text-slate-700" to="/teacher/users">Quay lại</Link>} /><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Tổng bài tập" value={progress.data?.totalAssignments ?? '-'} /><MetricCard label="Đã nộp" value={progress.data?.submittedAssignments ?? '-'} /><MetricCard label="Đã chấm" value={progress.data?.gradedAssignments ?? '-'} /><MetricCard label="Điểm TB" value={progress.data?.averageScore?.toFixed?.(1) ?? '-'} /></div><Card><h2 className="font-black text-slate-950">Nhật ký hoạt động</h2><div className="mt-3 divide-y divide-sky-50">{activityPage.items.map((item) => <div key={item.id} className="py-3"><p className="font-bold text-slate-900">{item.message}</p><p className="text-xs text-slate-500">{item.actorName} · {fmtDate(item.createdAt)}</p></div>)}{activityPage.items.length === 0 && <EmptyState title="Chưa có hoạt động" />}</div></Card></div>
}
