...
export const api = {
  ...
  createClass: (payload: { name: string; code: string; description?: string; levelFrom?: number; levelTo?: number; startDate?: string; endDate?: string }) => postApi<ClassItem>('/classes', payload),
  updateClass: (id: string, payload: Partial<{ name: string; description: string; levelFrom: number; levelTo: number; status: ClassStatus; startDate: string; endDate: string }>) => patchApi<ClassItem>(`/classes/${id}`, payload),
  ...
  createAssignmentsMulti: (payload: { classIds: string[]; title: string; description?: string; instruction?: string; dueAt?: string; maxScore?: number; allowResubmit?: boolean; skill?: string; fileId?: string; externalLink?: string }) => postApi<AssignmentItem[]>('/assignments', payload),
