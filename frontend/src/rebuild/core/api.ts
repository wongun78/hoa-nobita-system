...
export const api = {
  ...
  createAssignment: (classId: string, payload: { title: string; description?: string; instruction?: string; dueAt?: string; maxScore: number; status?: AssignmentItem['status']; allowResubmit?: boolean }) => postApi<AssignmentItem>(`/classes/${classId}/assignments`, payload),
